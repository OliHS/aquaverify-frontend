...
import { GLOSSARY_MARKETING_PAGES } from '../../utils/marketing-pages/glossaryPages.js';
import { MARKETING_LANGUAGES } from '../../utils/marketing-pages/shared.js';
import { GlossaryMarketingPageDocument } from '../../components/marketing/GlossaryMarketingPageDocument';
...
type GlossaryMarketingRouteProps = {
...
function getPageSummary(pageId: string | undefined, lang: Language) {
  ...
  const page = GLOSSARY_MARKETING_PAGES.find((item: any) => item.id === pageId);
...
export const GlossaryMarketingRoute: React.FC<GlossaryMarketingRouteProps> = ({ route }) => {
  ...
  const page = routeMatch?.family === 'glossary'
    ? GLOSSARY_MARKETING_PAGES.find((item: any) => item.id === routeMatch.pageId)
    : null;
...
export default GlossaryMarketingRoute;
